...
def draw_contour_ellipse(name, mask):
    ellipse, contour = getEllipse(mask)

    ret = np.copy(mask)
    ret = cv2.cvtColor(ret, cv2.COLOR_GRAY2BGR)
    cv2.drawContours(ret, contour, -1, (0, 255, 0), 3)

    cv2.circle(ret, (int(ellipse[0][0]), int(ellipse[0][1])), 7, (0, 0, 255), -1)  # 중심점 그리기

    ret = cv2.ellipse(ret, ellipse, (255, 0, 0), 2)
    cv2.imwrite(os.path.join(OUTPUT_DIR, f'{name}_contour_ellipse.png'), ret)

    print(f'angle after rotated: {ellipse[2]}')
...
if __name__ == "__main__":
    ...
    for image_path in tqdm.tqdm(input_images):
        ...
        cv2.imwrite(os.path.join(OUTPUT_DIR, 'blurred.png'), im)
        ...
        ellipses = [None for _ in range(len(masks))]  # ellipses
        ...
        cv2.imwrite(os.path.join(OUTPUT_DIR, 'mask_main.png'), mask_main)
        draw_contour_ellipse('mask_main', mask_main)
        ...
        # 이미지와 마스크 회전
        im_rotated = rotate_image(im, -(90 - angle), center=cen_pt)
        mask_rotated = rotate_image(mask_main, -(90 - angle), center=cen_pt)
        mask_rotated = np.reshape(mask_rotated, (*mask_rotated.shape[:2], 1))

        cv2.imwrite(os.path.join(OUTPUT_DIR, 'im_rotated.png'), im_rotated)
        cv2.imwrite(os.path.join(OUTPUT_DIR, 'mask_rotated.png'), mask_rotated)

        draw_contour_ellipse("mask_rotated", mask_rotated)
        ...
        # 물체 중심을 기준으로 잘라내기
        ellipse, _ = getEllipse(mask_rotated)
        ccen, rcen = map(int, map(round, ellipse[0]))  # 타원의 중심점의 좌표를 정수로 변환
        rad = int(ceil(rad))  # 반지름을 올림해서 정수로 변환
        rect_main = [ccen - rad, rcen - rad, 2 * rad, 2 * rad]  # 이미지를 잘라낼 영역의 좌표 계산

        # crop
        im_crop = _crop_image(im_rotated, rect_main, args.const_border, value=255)
        mask_crop = _crop_image(mask_rotated, rect_main, True, value=0)
        ...
        # mask_crop = cv2.resize(mask_crop, (args.size, args.size), interpolation=cv2.INTER_LINEAR)
        masked_crop = cv2.resize(masked_crop, (args.size, args.size), interpolation=cv2.INTER_AREA)
